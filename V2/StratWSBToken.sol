...
contract StratWSBToken is Ownable, ReentrancyGuard, Pausable {
        ...
        returns (uint256)
    {
        ...
        //user only pays fee if the 
        if(lastUserDepositTime[_userAddress] < block.timestamp.add(WITHDRAW_FEE_PERIOD)) {
            IERC20(wantAddress).safeTransfer(feeReceiver, feeAmount);
